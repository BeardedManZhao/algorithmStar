...
public class ColumnIntegerVector extends IntegerVector {
     ...
     */
    @Override
    public ColumnIntegerVector reverseLR(boolean isCopy) {
        if (isCopy) {
            return ColumnIntegerVector.parse(
                    this.vectorName,
                    ASMath.arrayReverse(this.getFieldNames().clone()),
                    ASMath.arrayReverse(this.copyToNewArray())
            );
        } else {
            ASMath.arrayReverse(this.getFieldNames());
