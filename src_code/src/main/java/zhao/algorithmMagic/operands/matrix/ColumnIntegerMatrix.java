...
public class ColumnIntegerMatrix extends IntegerMatrix implements RCNOperands<IntegerVector, int[]> {
    ...
    protected static void ex(Random random1, int[][] res, String[] rowNames, int maxIndex, int i) {
        ...
        if (index >= 0 && index < rowCount) {
            ...
            if (b1 && b2) {
                ...
                return ColumnIntegerMatrix.parse(
                        this.Field1.clone(), res_f2.toArray(new String[0]), res.toArray(new int[0][])
                );
            } else if (b2) {
                // 说明第 1 字段不需要添加数据
                deleteRelatedFunction(thresholdLeft, thresholdRight, ints, mid, res, res_f2, Field2);
                ...
                return ColumnIntegerMatrix.parse(
                        this.Field1.clone(), null, res.toArray(new int[0][])
                );
            } else {
                // 说明都不需要字段名数据
     ...
     */
    @Override
    public ColumnIntegerMatrix shuffle(long seed) {
        // 行是否无字段
     ...
     */
    @Override
    public ColumnIntegerMatrix reverseLR(boolean isCopy) {
        if (!isCopy) {
     ...
     */
    @Override
    public ColumnIntegerMatrix reverseBT(boolean isCopy) {
        if (isCopy) {
            return ColumnIntegerMatrix.parse(
